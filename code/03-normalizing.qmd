...
str(extracted_slim)
```
## Make new dataframes separately for DNA and RNA
```{r}
...
Let's write a code that calculates the volumes needed for us
### RNA
```{r}
...
# Use mutate to create a new column and calculate the volume of eluted RNA in uL (V1) needed to normalize RNA concentration to 25ng/uL. If starting concentration is less than 25, make output 'NA'.
extracted_rna <- extracted_rna %>%
  mutate(rna_elut_vol_norm = ifelse(
    qubit_rna_avg < 25,
    NA,
    round((C1rna * V2rna) / qubit_rna_avg, digits = 0)
  )) %>%
...
extracted_rna <- extracted_rna %>%
  mutate(rna_nucfreewater_vol_norm = (V2rna - rna_elut_vol_norm)) %>%
